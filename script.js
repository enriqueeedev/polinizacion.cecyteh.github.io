/**
 * ================================================================
 *  EL JARDÍN QUE DA VIDA — script.js
 *  Lógica interactiva: navbar, scroll, animaciones,
 *  modo oscuro, galería, audio y botones flotantes.
 * ================================================================
 */
...
function initNavbar() {
  ...
  // Aplicar clase "scrolled" cuando se pasa de 60px
  const onScroll = () => {
...
function initTheme() {
  ...
  const KEY  = 'jardin-theme'; // clave en localStorage
...
function initScrollReveal() {
  ...
  const observer = new IntersectionObserver(
    ...
    {
      threshold: 0.12,  // 12% visible antes de animar
...
function initGallery() {
  ...
  // Construir array de imágenes para navegación
  let galleryData = [];
  let currentIndex = 0;
  ...
  function navigate(direction) {
    ...
    // Animación de fade rápido al cambiar
...
function initAudioButtons() {
  ...
  buttons.forEach(btn => {
    // Cada botón puede tener un audio diferente definido en data-src
    const src = btn.getAttribute('data-src');
    if (!src) return;

    btn.addEventListener('click', () => {
      // Si el audio actual es el mismo, pausar/reanudar
      ...
      // Pausar el audio anterior si existía
      ...
      // ✅ El src del audio viene del atributo data-src del botón
  ...
  function updateBtnText(btn, playing) {
    // Conservar el SVG, solo cambiar texto
    const svg = btn.querySelector('svg');
    btn.textContent = playing ? 'Pausar audio' : btn.getAttribute('data-original-text') || 'Reproducir';
    if (svg) btn.prepend(svg);
  }

  // Guardar textos originales de los botones al iniciar
  buttons.forEach(btn => {
    // Extraer solo el texto sin el ícono SVG
...
function initBgMusic() {
  ...
  // ✅ El src del audio de fondo está en el HTML: audio/musica-fondo.mp3
  let isPlaying = false;
...
/* ----------------------------------------------------------------
   11. SMOOTH SCROLL PARA LINKS CON #
      (para navegadores que no soportan scroll-behavior CSS)
   ---------------------------------------------------------------- */
...
/* ----------------------------------------------------------------
   12. UTILIDAD: LAZY LOADING DE IMÁGENES (fallback)
       Los navegadores modernos lo hacen nativamente con loading="lazy"
       pero por si acaso también aplicamos IntersectionObserver.
   ---------------------------------------------------------------- */
...
if (heroSection) {
  window.addEventListener('scroll', () => {
    ...
    // Mover fondo del hero lentamente en sentido contrario
